...
class FinestraMantenimentComarques : JFrame() {
    ...
    fun agafarComarques(): ArrayList<Comarca> {
        ...
        val q = s.createQuery("from Comarca order by nomC")
        for (c in q.list())
            llista.add(c as Comarca)
        return llista
    }

    fun visComarca() {
        // Mètode per a visualitzar la comarca marcada per l'índex que ve com a paràmetre
        val c = llistaComarques.get(indActual)
        nomComarca.text = c.nomC
        nomProvincia.text = c.provincia
        controlBotons()
    }
    ...
    fun controlBotons() {
        ...
        seguent.isEnabled = (indActual!=(llistaComarques.size-1))
    }

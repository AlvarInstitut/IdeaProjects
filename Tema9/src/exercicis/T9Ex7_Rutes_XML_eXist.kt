...
class T9Ex7_Rutes_XML_eXist : JFrame() {
    ...
    fun VisRuta() {
        val r = rs?.`object` as Element
        qNom.text = r.getElementsByTagName("nom").item(0).textContent
        qDesn.text = r.getElementsByTagName("desnivell").item(0).textContent
        qDesnAcum.text = r.getElementsByTagName("desnivellAcumulat").item(0).textContent
        plenar_taula(r.getElementsByTagName("punts").item(0) as Element)
